...
function loadOnlineUsers() {
    ...
    function renderUserLists() {
        ...
        if (groupUsersList) {
            groupUsersList.innerHTML = validUsers.length === 0
    ...
    rtdb.ref('presence').on('value', (snapshot) => {
        ...
        snapshot.forEach((child) => {
            const p = child.val();
            if (p && p.uid) presenceData[p.uid] = p;
        });
        renderUserLists();
    });
}
