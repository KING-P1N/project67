...
function loadOnlineUsers() {
    ...
    presenceRef.on('value', async (snapshot) => {
        ...
        // Cross-check each presence entry against Firestore users collection.
        // If the user doc no longer exists (deleted), remove the stale presence entry.
        const validUsers = [];
        await Promise.all(rawUsers.map(async (u) => {
            try {
                ...
                if (doc.exists) {
                    ...
                    console.log('🧹 Removed stale presence for:', u.uid);
                }
            } catch (e) {
                // If check fails, keep the user to avoid false removals
                validUsers.push(u);
            }
        }));
