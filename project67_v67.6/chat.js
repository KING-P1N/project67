...
function loadOnlineUsers() {
    ...
    usersRef.on('value', (usersSnapshot) => {
        presenceRef.on('value', (presenceSnapshot) => {
            const onlineMap = {};
            presenceSnapshot.forEach((child) => {
                const p = child.val();
                if (p.isOnline) {
                    onlineMap[p.uid] = true;
                }
            });
            ...
            usersSnapshot.forEach((child) => {
                const u = child.val();
                if (u.uid && u.uid !== currentUser.uid) {
                    u.isOnline = !!onlineMap[u.uid];
                    validUsers.push(u);
                }
            });

            // Sort so online users are at the top
            validUsers.sort((a, b) => (b.isOnline ? 1 : 0) - (a.isOnline ? 1 : 0));
            ...
            // Private chat sidebar — show ALL users (online + offline)
            ...
            console.error("RTDB Presence read failed:", error);
        });
    }, (error) => {
        console.error("RTDB Users read failed:", error);
    });
}
...
function sendGroupMessage(event) {
    ...
    const cachedUsername = localStorage.getItem('userUsername') || (currentUser ? currentUser.displayName : null) || currentUser.email.split('@')[0];
...
function displayGroupMessage(message) {
    ...
    const timestamp = msgTime.toLocaleTimeString([], timeOptions);
    const messageClass = isOwn ? 'message-own' : 'message-other';
    const senderName = isOwn ? 'You' : (message.username || message.email);

    display.innerHTML += `
        <div class="message-wrapper ${isOwn ? 'wrapper-own' : 'wrapper-other'}">
