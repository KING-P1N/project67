...
auth.onAuthStateChanged((user) => {
    if (user) {
        ...
        // Mark user as online and set onDisconnect
...
function markUserOnline(uid, email) {
    ...
    // Get username from Firestore then update presence
...
function loadOnlineUsers() {
    ...
    presenceRef.on('value', (snapshot) => {
        ...
        // Group chat sidebar - show all users with online/offline status
        ...
        if (allUsers.length === 0) {
            ...
            groupUsersList.innerHTML = allUsers.map(u => 
                `<div class="user-item">
                    ${u.isOnline ? '🟢' : '⚫'} ${u.email}
        ...
        if (allUsers.length === 0) {
            ...
            privateUsersList.innerHTML = allUsers.map(u => 
...
function executeKillswitch() {
    // Only delete messages, never touch presence
    rtdb.ref('messages').remove().then(() => {
        alert('🔴 ALL MESSAGES DELETED');
        document.getElementById('killswitchNormal').style.display = 'block';
        document.getElementById('killswitchCountdown').style.display = 'none';
        // Reload chat display without full page reload
